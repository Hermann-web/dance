...
if [[ -z "${ROOT_DIR}" ]]; then
  echo "Usage: $0 <cpsc2021_root> <record1> [record2 ...]" >&2
  exit 1
fi
if [[ ${#RECORDS[@]} -eq 0 ]]; then
  echo "Provide at least one CPSC2021 record stem." >&2
  exit 1
fi
...
OUT_DIR="results/ecg/cpsc2021/${STAMP}"
mkdir -p "${OUT_DIR}"
LOG="${OUT_DIR}/train.log"

echo "Running CPSC2021 training. Logs: ${LOG}"
